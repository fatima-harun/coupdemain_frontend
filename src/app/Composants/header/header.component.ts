...
export class HeaderComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.isAdmin = user && user.roles.some((role: { name: string; }) => role.name === 'admin');
  ...
  fetchNotifications() { this.notificationService.getNotifications().subscribe((notifications) => {
   ...
   onMarkAsRead(notificationId: number) {
    this.notificationService.markAsRead(notificationId).subscribe( (response) => {
      ...
      if (notification) { notification.read = true; this.unreadNotificationsCount--;
